...
public class IceStormSpell extends BaseSpell {
    public IceStormSpell(String gesture, ControlEntity caster) {
        super("Ice Storm", gesture, SpellType.DAMAGING, caster, false);
    }
    ...
    public void action(ControlEntity target) {
        ...
        for (ControlEntity entity : entities) {
            // 直接杀死火元素
            if (entity instanceof Monster && ((Monster) entity).type == ElementType.FIRE) {
                entity.setHealth(0);
            }
            // 这里借用一下怪物的伤害技能 此技能是整个游戏唯一带属性的攻击技能 用于元素攻击
            entity.addTempSpell(new MonsterDamageSpell(caster, 5, ElementType.ICE));
        }
    }
}
